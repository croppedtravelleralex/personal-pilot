    ...
    let summary = match kind {
        ...
        "m4_profile_facade" => {
            ...
            let failed = summary
                ...
                .and_then(Value::as_i64)
                .unwrap_or_default();
            let typed_desktop_wrapper = summary
                .and_then(|item| value_text(item, "typedDesktopWrapper"))
                .unwrap_or_else(|| "missing".to_string());
            let browser_fetch_fallback = summary
                .and_then(|item| value_text(item, "browserFetchFallback"))
                .unwrap_or_else(|| "missing".to_string());
            let dynamic_binding_removed = summary
                .and_then(|item| value_text(item, "dynamicBindingRemoved"))
                .unwrap_or_else(|| "unknown".to_string());
        ...
        "m4_automation_facade" => {
            let summary = value.get("summary");
            let failed = summary
                .and_then(|item| item.get("failed"))
                .and_then(Value::as_i64)
                .unwrap_or_default();
            let page_facade_usage = summary
                .and_then(|item| value_text(item, "pageFacadeUsage"))
                .unwrap_or_else(|| "missing".to_string());
            let direct_wails_import_removed = summary
                .and_then(|item| value_text(item, "directWailsImportRemoved"))
                .unwrap_or_else(|| "unknown".to_string());
        ...
        "m4_browser_runtime_facade" => {
            let summary = value.get("summary");
            let failed = summary
                .and_then(|item| item.get("failed"))
                .and_then(Value::as_i64)
                .unwrap_or_default();
            let browser_runtime_facade_usage = summary
                .and_then(|item| value_text(item, "browserRuntimeFacadeUsage"))
                .unwrap_or_else(|| "missing".to_string());
            let direct_runtime_imports_removed = summary
                .and_then(|item| value_text(item, "directRuntimeImportsRemoved"))
                .unwrap_or_else(|| "unknown".to_string());
            let desktop_runtime_wrapper = summary
                .and_then(|item| value_text(item, "desktopRuntimeWrapper"))
                .unwrap_or_else(|| "missing".to_string());
            format!(
...
fn evidence_level_from_report(kind: &str, status: &str, value: &Value) -> String {
    match (kind, status) {
        ...
        ("m4_profile_facade", "passed_profile_facade_contract") => {
        ...
        ("m4_automation_facade", "passed_automation_facade_contract") => {
            "automation_facade_contract_partial"
        }
        ("m4_automation_facade", "failed_automation_facade_contract") => {
        ...
        ("m4_browser_runtime_facade", "passed_browser_runtime_facade_contract") => {
            "browser_runtime_facade_contract_partial"
        }
        ("m4_browser_runtime_facade", "failed_browser_runtime_facade_contract") => {
...
fn evidence_next_action(
    ...
    let action = match (kind, status) {
        ...
        ("m4_profile_facade", "passed_profile_facade_contract")
        | ("m4_profile_facade", "failed_profile_facade_contract") => value
            .get("summary")
            .and_then(|summary| value_text(summary, "nextAction"))
            .or_else(|| {
                Some(
                    "Run scripts/m4_profile_facade_gate.ps1, then continue shrinking remaining workbench/core bridge APIs separately."
                        .to_string(),
                )
            }),
        ("m4_behavior_preset_facade", "passed_behavior_preset_facade_contract")
        ...
        ("m4_automation_facade", "passed_automation_facade_contract")
        | ("m4_automation_facade", "failed_automation_facade_contract") => value
            .get("summary")
            .and_then(|summary| value_text(summary, "nextAction"))
            .or_else(|| {
                Some(
                    "Run scripts/m4_automation_facade_gate.ps1, then continue shrinking remaining monitor/app shell/browser runtime bridge APIs separately."
                        .to_string(),
                )
            }),
        ("m4_app_shell_facade", "passed_app_shell_facade_contract")
        ...
        ("m4_browser_runtime_facade", "passed_browser_runtime_facade_contract")
        | ("m4_browser_runtime_facade", "failed_browser_runtime_facade_contract") => value
            .get("summary")
            .and_then(|summary| value_text(summary, "nextAction"))
            .or_else(|| {
                Some(
                    "Run scripts/m4_browser_runtime_facade_gate.ps1, then continue shrinking remaining settings/core/proxy runtime bridge APIs separately."
                        .to_string(),
                )
            }),
        ("m4_runtime_facade", "passed_runtime_facade_contract")
...
mod tests {
    ...
    fn evidence_report_history_failure_reason_category_covers_known_blockers() {
        let cases = [
            ...
            (
                "m4_profile_facade",
                "passed_profile_facade_contract",
                None,
                "none",
            ),
            (
                "m4_profile_facade",
                "failed_profile_facade_contract",
                None,
                "profile_facade_contract_incomplete",
            ),
            (
            ...
            (
                "m4_automation_facade",
                "passed_automation_facade_contract",
                None,
                "none",
            ),
            (
                "m4_automation_facade",
                "failed_automation_facade_contract",
                None,
                "automation_facade_contract_incomplete",
            ),
            (
            ...
            (
                "m4_browser_runtime_facade",
                "passed_browser_runtime_facade_contract",
                None,
                "none",
            ),
            (
                "m4_browser_runtime_facade",
                "failed_browser_runtime_facade_contract",
                None,
                "browser_runtime_facade_contract_incomplete",
            ),
            (
    ...
    fn evidence_report_history_reads_runtime_task_and_taxonomy_reports() {
        ...
        fs::write(
            reports_root
                .join("m4-profile-facade")
                .join("m4-profile-facade-gate-test.json"),
            serde_json::json!({
                "schemaVersion": "m4_profile_facade_gate_v1",
                "generatedAt": "2026-05-30T00:05:48Z",
                "status": "passed_profile_facade_contract",
                "failureReason": "",
                "summary": {
                    "failed": 0,
                    "typedDesktopWrapper": "present",
                    "browserFetchFallback": "preserved",
                    "dynamicBindingRemoved": "yes",
                    "nextAction": "Continue shrinking remaining workbench/core bridge APIs without claiming tauriWailsBridge removal."
                }
            })
            .to_string(),
        )
        ...
        fs::write(
            reports_root
                .join("m4-automation-facade")
                .join("m4-automation-facade-gate-test.json"),
            serde_json::json!({
                "schemaVersion": "m4_automation_facade_gate_v1",
                "generatedAt": "2026-05-30T00:05:50Z",
                "status": "passed_automation_facade_contract",
                "failureReason": "",
                "summary": {
                    "failed": 0,
                    "pageFacadeUsage": "present",
                    "directWailsImportRemoved": "yes",
                    "browserApiFacade": "present",
                    "nextAction": "Continue shrinking remaining monitor/app shell/browser runtime bridge APIs without claiming tauriWailsBridge removal."
                }
            })
            .to_string(),
        )
        ...
        fs::write(
            reports_root
                .join("m4-browser-runtime-facade")
                .join("m4-browser-runtime-facade-gate-test.json"),
            serde_json::json!({
                "schemaVersion": "m4_browser_runtime_facade_gate_v1",
                "generatedAt": "2026-05-30T00:05:53Z",
                "status": "passed_browser_runtime_facade_contract",
                "failureReason": "",
                "summary": {
                    "failed": 0,
                    "browserRuntimeFacadeUsage": "present",
                    "directRuntimeImportsRemoved": "yes",
                    "desktopRuntimeWrapper": "present",
                    "nextAction": "Continue shrinking remaining settings/core/proxy runtime bridge APIs without claiming tauriWailsBridge removal."
                }
            })
            .to_string(),
        )
        ...
        let m4_profile = history
            .reports
            .iter()
            .find(|report| report.kind == "m4_profile_facade")
            .expect("m4 profile facade report");
        assert_eq!(m4_profile.status, "passed_profile_facade_contract");
        assert_eq!(m4_profile.evidence_level, "profile_facade_contract_partial");
        assert_eq!(m4_profile.failure_reason_category, "none");
        assert_eq!(m4_profile.risk_level, "partial");
        assert!(m4_profile.summary.contains("typedDesktopWrapper=present"));
        assert!(m4_profile
            .summary
        ...
        let m4_automation = history
            .reports
            .iter()
            .find(|report| report.kind == "m4_automation_facade")
            .expect("m4 automation facade report");
        assert_eq!(m4_automation.status, "passed_automation_facade_contract");
        assert_eq!(
            m4_automation.evidence_level,
            "automation_facade_contract_partial"
        );
        assert_eq!(m4_automation.failure_reason_category, "none");
        assert_eq!(m4_automation.risk_level, "partial");
        assert!(m4_automation.summary.contains("pageFacadeUsage=present"));
        ...
        let m4_browser_runtime = history
            .reports
            .iter()
            .find(|report| report.kind == "m4_browser_runtime_facade")
            .expect("m4 browser runtime facade report");
        assert_eq!(
            m4_browser_runtime.status,
            "passed_browser_runtime_facade_contract"
        );
        assert_eq!(
            m4_browser_runtime.evidence_level,
            "browser_runtime_facade_contract_partial"
        );
        assert_eq!(m4_browser_runtime.failure_reason_category, "none");
        assert_eq!(m4_browser_runtime.risk_level, "partial");
        assert!(m4_browser_runtime
            .summary
            .contains("browserRuntimeFacadeUsage=present"));
        assert!(m4_browser_runtime
            .summary
