...
export function OverviewPage() {
  ...
  return (
    <div className="page-stack">
      ...
      <Panel
        title="Evidence Report History"
        subtitle="Local release, provider, portability, taxonomy, and distribution evidence generated by smoke scripts"
      >
        {runtime.evidenceReports && runtime.evidenceReports.reports.length > 0 ? (
          <div className="record-list">
            {runtime.evidenceReports.reports.slice(0, 5).map((report) => (
              <article className="record-card record-card--compact" key={`${report.kind}-${report.reportId}`}>
                <div className="record-card__top">
                  <div>
                    <strong>{formatStatusLabel(report.kind)}</strong>
                    <p className="record-card__subline">{report.reportId}</p>
                  </div>
                  <span className={`badge badge--${getTaskBadgeTone(report.status)}`}>
                    {formatStatusLabel(report.status)}
                  </span>
                </div>
                <p className="record-card__content record-card__content--muted">{report.summary}</p>
                {report.failureReason ? (
                  <p className="record-card__content record-card__content--muted">{report.failureReason}</p>
                ) : null}
                <div className="record-card__footer">
                  <span>{formatRelativeTimestamp(report.generatedAt)}</span>
          ...
          <EmptyState
            ...
            detail="Run the local smoke scripts to populate release, provider, portability, taxonomy, and distribution evidence history."
          />
        )}
      </Panel>
