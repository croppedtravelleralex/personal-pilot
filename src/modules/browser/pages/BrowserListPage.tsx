...
export function BrowserListPage() {
  ...
  const [_proxyErrorMsg, _setProxyErrorMsg] = useState('')
  ...
  const syncRecordingProfiles = async () => {
    try {
      const status = await fetchRecordingStatus()
      setRecordingProfiles(new Set(status.profileIds))
    } catch {
      // Keep the last known state if the backend status endpoint is temporarily unavailable.
    }
  }
  ...
  const handleValidateCorePath = async () => {
    ...
    const result = await validateBrowserCorePath(coreForm.corePath, coreForm.kind)
  ...
  const coreColumns: TableColumn<BrowserCore>[] = [
    ...
    { key: 'kind', title: '引擎', render: (value) => coreKindLabel(String(value || '')) },
        ...
        <div className="space-y-4">
          ...
          <FormItem label="浏览器引擎" required>
            <select
              ...
              onChange={e => { setCoreForm(prev => ({ ...prev, kind: normalizeCoreKind(e.target.value) })); setCoreValidation(null) }}
      ...
      >
        <div className="space-y-3">
          <div className="flex items-start gap-3 p-3 rounded-lg bg-[var(--color-bg-secondary)]">
            ...
            <p className="text-sm text-[var(--color-text-primary)]">{_proxyErrorMsg}</p>
          </div>
