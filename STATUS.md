...
3. **推进真实 Lightpanda 执行路径深化**，当前已完成 execution-path hardening v1、runner 可观测性 / artifact 质量 v1、指纹真实消费边界 v1、真实能力扩展边界 v1、action contract visibility v1，并已落下最小新动作 `get_html` bounded v1：复用当前 fetch-style 路径输出 `html_preview`。
...
- **改进建议：** 下一步最值得做的是 **围绕 `get_html` bounded v1 继续补结果边界与摘要质量，但不要立刻跳去 screenshot / script 这类更重动作**。
