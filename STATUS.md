...
3. **推进真实 Lightpanda 执行路径深化**，当前已完成 execution-path hardening v1、runner 可观测性 / artifact 质量 v1、指纹真实消费边界 v1、真实能力扩展边界 v1、action contract visibility v1，并确认下一最小新动作候选应优先选择 `get_html`，而不是更重的 screenshot / script。
...
- **改进建议：** 下一步最值得做的是 **围绕 `get_html` 候选验证最小实现路径，先确认是否能安全复用当前 fetch-style 路径，再决定是否真正落代码**。
