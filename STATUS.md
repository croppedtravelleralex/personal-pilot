...
3. **推进 provider 级 refresh 范围收窄最小实现**，当前已落下 `provider_risk_snapshots.version`、`proxies.provider_risk_version_seen` 与 `provider_scope_flip -> 当前 proxy 懒更新` 的第一版，不与 provider_region 一起上。
...
- **改进建议：** 下一步最值得做的是 **继续验证 provider risk version / seen 第一版在真实链路中的收益，并决定何时再把 provider_region 纳入同类语义**。
