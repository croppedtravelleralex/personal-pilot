...
3. **推进绝对指纹优先第一批真实实现**，当前已补强并发预算行为回归测试：把 claim 候选挑选逻辑抽成纯函数并补单元测试，明确验证 heavy 满额时不会堵死 light/medium，避免系统退化成伪串行。
...
- **改进建议：** 下一步最值得做的是 **把 fingerprint consistency / budget 决策进一步接入 task result / explain summary，形成更完整的上层 explain 闭环**。
