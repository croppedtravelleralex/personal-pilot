...
3. **推进绝对指纹优先第一批真实实现**，当前已落下 `fingerprint_policy` 第一版：把指纹字段优先级分层（L1/L2/L3）与默认性能预算标签（light/medium/heavy）收成可复用模块，作为后续一致性检查与性能预算实现的基础。
...
- **改进建议：** 下一步最值得做的是 **基于 `fingerprint_policy` 继续落指纹-代理-地区一致性检查，并让性能预算规则开始挂到真实实现上**。
