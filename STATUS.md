...
3. **推进 provider 级 refresh 范围收窄稳定化**，当前已完成 providerRegion 进入条件与第二阶段边界定义；selection 继续不动，providerRegion 继续延后，下一阶段将转入“是否满足 providerRegion 进入条件”的验证。
...
- **改进建议：** 下一步最值得做的是 **围绕 providerRegion 进入条件补最小验证样本，而不是直接开做 providerRegion 实现；selection 继续不动**。
