...
function Get-PersonalPilotResidualProcesses {
  @(
...
$targets = @(Get-PersonalPilotResidualProcesses | Sort-Object ProcessId -Descending)
foreach ($p in $targets) {
  try { Stop-Process -Id $p.ProcessId -Force -ErrorAction Stop } catch {}
}
Start-Sleep -Milliseconds 400
$left = @(Get-PersonalPilotResidualProcesses)
if ($left.Count -gt 0) {
...
`
	...
	tempFile, err := os.CreateTemp("", "personal-pilot-cleanup-*.ps1")
	if err != nil {
