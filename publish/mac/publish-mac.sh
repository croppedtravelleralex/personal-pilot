...
ZIP_NAME="PersonalPilot-${VERSION}-macos-${ARCH}.zip"
APP_EXPORT="$OUTPUT_DIR/PersonalPilot-${VERSION}-macos-${ARCH}.app"
STAGE_DIR="$STAGING_ROOT/$TARGET"
APP_STAGE="$STAGE_DIR/Personal Pilot.app"
...
echo "========================================"
echo "  Personal Pilot macOS Publish"
echo "========================================"
echo "Target : $TARGET"
...
if [[ "$SKIP_BUILD" -ne 1 ]]; then
  ...
  (
    cd "$ROOT_DIR"
    wails build -s -platform "darwin/$ARCH" -o personal-pilot
  )
else
