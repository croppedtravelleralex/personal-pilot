...
APP_BIN="$ROOT_DIR/build/bin/personal-pilot"
WAILS_CONFIG="$ROOT_DIR/wails.json"
CHROME_README_SRC="$ROOT_DIR/chrome/README.md"
APP_PACKAGE_NAME="personal-pilot"
APP_BINARY_NAME="personal-pilot"
APP_ICON_NAME="personal-pilot"
APP_DESKTOP_ID="personal-pilot.desktop"
APPSTREAM_ID="personal-pilot"
APP_NAME="Personal Pilot"
APP_SUMMARY="Multi-profile browser launcher with proxy-pool management"
APP_MAINTAINER="Personal Pilot Team"
APP_MAINTAINER_EMAIL="contact@antblack.dev"
APP_HOMEPAGE="https://github.com/croppedtravelleralex/personal-pilot"
BUILD_DATE_UTC="$(date -u +%F)"
ICON_SIZES=(16 24 32 48 64 128 256 512)

echo "========================================"
echo "  Personal Pilot Linux Publish"
echo "========================================"
echo "Target : $TARGET"
...
if [[ "$SKIP_BUILD" -ne 1 ]]; then
  ...
  (
    cd "$ROOT_DIR"
    wails build -s -platform "linux/$ARCH" -o personal-pilot
  )
else
...
cp "$APP_BIN" "$APP_STAGE/personal-pilot"
...
chmod +x "$APP_STAGE/personal-pilot" "$APP_STAGE/bin/xray" "$APP_STAGE/bin/sing-box"
...
cp "$APP_STAGE/personal-pilot" "$INSTALL_ROOT/personal-pilot"
...
chmod +x "$INSTALL_ROOT/personal-pilot" "$INSTALL_ROOT/bin/xray" "$INSTALL_ROOT/bin/sing-box"
...
Name=${APP_NAME}
Comment=${APP_SUMMARY}
Exec=/opt/personal-pilot/${APP_BINARY_NAME}
TryExec=/opt/personal-pilot/${APP_BINARY_NAME}
Icon=${APP_ICON_NAME}
StartupWMClass=Personal-Pilot
Terminal=false
Type=Application
StartupNotify=true
...
<component type="desktop-application">
  ...
  <description>
    <p>Personal Pilot manages isolated browser profiles, proxy binding, and local environment configuration for multi-account workflows.</p>
...
Description: ${APP_NAME} desktop app
 ...
 Personal Pilot manages isolated browser profiles, proxy binding, and
...
#!/bin/sh
set -e
ln -sf /opt/personal-pilot/personal-pilot /usr/bin/personal-pilot
chmod +x /opt/personal-pilot/personal-pilot /opt/personal-pilot/bin/xray /opt/personal-pilot/bin/sing-box || true
