...
- external browser integration 已有计划和 contract 边界，但不是完整 runtime depth；主仓库仍不托管 Chromium/Firefox fork。2026-05-27 已在 `docs/18-external-browser-integration-plan.md` 固化并深化 Camoufox `90` 分方案；2026-05-28 已把 Camoufox 作为 `browser_cores.kind=camoufox` 接入主线内核管理、路径校验、实例二进制解析和启动参数分发，并在主线 UI 三个内核入口可选。Rust runner skeleton 仍显式 `runner_not_implemented`、`browser_launch_attempted=false`，所以只能写成 core-manager integrated / runtime smoke pending，不能写成真实 Camoufox task runtime-ready。`scripts/camoufox_smoke.ps1 -AllowBlocked` 当前状态为 `contract_ready_runtime_smoke_required`。
