...
- Validation Board 已落地到桌面导航：8 类 evidence（detector / leak / DNS / WebRTC / canvas / audio / worker / transport）按 declared / applied / observed 分层展示；DNS/transport observed 首批 native collector、WebRTC/leak observed contract warning、JSON report、report history 和 profile-level evidence export 已接入，仍不代表完整 browser-scoped leak/WebRTC/canvas/audio 闭环。
...
- Validation Board 已有前端 MVP，并已接入 DNS/transport 首批 observed native collector、本地 JSON report、history list 和 profile-level evidence export；后续需把 WebRTC/leak 从 contract warning 升级为 browser-scoped collector，并扩展 canvas/audio collector。
...
1. 将 WebRTC/leak observed contract warning 升级为 browser-scoped collector，并扩展 canvas/audio collector
