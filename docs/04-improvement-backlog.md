...
| Fingerprint | 从 `12` runtime projected fields 扩展 runtime depth | 已扩到 `26` projected fields（`25` control-supported + derived `platform`）；P7 已新增 WebRTC/canvas/audio/leak observed audit；P14 已新增 `450` fingerprint signal taxonomy seed；2026-05-28 已新增环境注入编译器和单次 CDP 新文档注入入口，覆盖 browser API/canvas/timezone/WebGL/media devices 基础 hook | declared / applied / observed 可解释且可验证；taxonomy seed 不得计入 observed proof；下一步补 audio/fonts/WebRTC/consistency matrix 与 profile-browser observed evidence |
...
| Behavior | 从 `13` primitives 扩展 replayable `450+` taxonomy | P10 behavior audit contract 已落地；P14 新增 `450` behavior event taxonomy seed 和 audit path；2026-05-28 已落地 Phase 2 P0 的 Fitts Law 轨迹、粉噪、四段式点击、双击、拖拽和右键菜单计划模型 | 事件数量来自真实 taxonomy；taxonomy seed 仍需 replay runtime 证据才能算交付；下一步补 typing/scroll P1 并接入执行器证据 |
...
| Runtime | headed realism、kernel strategy、adapter boundary | adapter/release smoke contract、P11 measurement pending visibility 和 P21 runtime adapter evidence gate 已落地；P14 新增 release performance smoke 脚本，2026-05-27 根目录 `personal-pilot-tauri.exe` 复测 warning：`10274ms / 672MB / 22 processes`，超过 `2000ms / 220MB / 4 processes` 默认预算；headed runtime 未实现 | 有稳定 adapter contract、真实 runtime 验证证据，并给出性能超预算 mitigation |
| Runtime | Camoufox 单任务生产级 Runner | 2026-05-27 已在 `docs/18-external-browser-integration-plan.md` 固化并深化 `90` 分方案；当前已有 skeleton / settings / capability 合同和 `scripts/camoufox_smoke.ps1` 验证入口，但 runner 仍显式 `runner_not_implemented`、`browser_launch_attempted=false`，状态只能记为 contract-ready / runtime blocked；subagent 分析尝试因本地 distributor `503 Service Unavailable` 降级为主线程只读扫描 | Camoufox 可配置、可检测、可执行、可取消、可清理、可回显、可诊断；artifact 落盘；基础 profile/proxy 映射；Lightpanda 不回归；默认不常驻、不预热、并发默认 1；`scripts/camoufox_smoke.ps1`、release idle smoke 与 Camoufox task-run 性能报告均通过或记录明确例外；Win11/Tauri enforcement 通过 |
