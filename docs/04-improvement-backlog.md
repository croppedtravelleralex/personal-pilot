...
| Release | 单一 exe / 单一 UI 收敛 | 2026-05-25 第一版已生成并覆盖根目录 `personal-pilot-tauri.exe`，Tauri release 也产出同名 exe；已核验 core bridge 返回真实 `4/95/3` 数据；2026-05-26 已清理根目录旁路 exe、旧 release exe 和仓库内 `gateway-ui/` | 构建、脚本、文档、smoke 和外部分发只认 `personal-pilot-tauri.exe`；旁路 exe/UI 不再参与构建、验证或分发 |
...
| Runtime | headed realism、kernel strategy、adapter boundary | adapter/release smoke contract、P11 measurement pending visibility 和 P21 runtime adapter evidence gate 已落地；P14 新增 release performance smoke 脚本，2026-05-26 根目录 `personal-pilot-tauri.exe` 复测 warning：`2437ms / 465MB / 9 processes`，超过 `2000ms / 220MB / 4 processes` 默认预算；headed runtime 未实现 | 有稳定 adapter contract、真实 runtime 验证证据，并给出性能超预算 mitigation |
...
| UI consolidation | 第二套 UI / 旁路 exe 清理与主线 API 收口 | 根目录旁路 exe、旧 release exe 和仓库内 `gateway-ui/` 已清理；Gateway dashboard UI 如需使用必须通过 `GATEWAY_UI_DIR` 指向外部目录；2026-05-27 已把 `dashboard/profile/monitor` 收口到 `services/desktop.ts` typed wrapper 或模块 facade，并为事件监控 history 补上 `300ms` debounce 与 stale-result 保护；同日已修复 `/browser/detail/:profileId`、`/browser/edit/:profileId`、`/browser/copy/:profileId` 仍误读 `id` 的回归；剩余风险主要在 `synchronizer/api.ts` 和 browser 模块动态 facade | 后续深度测试确认主线 UI 操作、Wails 兼容 API、report/history 和 Settings/runtime 细节无回归；继续把 `src/modules/synchronizer/api.ts` 与 browser 模块动态 RPC 逐步显式化为 typed facade |
