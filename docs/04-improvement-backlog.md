...
| Workflow task center | M4 本地任务创建/运行/状态检查闭环 | 2026-06-01 已把 scheduler runtime state 持久化到 SQLite：`status`、`last_run_at`、`last_error`、`retry_count`；Automation 任务页运行后刷新并展示步骤数、上次运行、错误和重试信息；迁移版本 13 可修复旧库缺列 | 继续补 workflow execution detail、task run history、cancel/pause operator controls、provider dry-run action 和 UI/API typed facade；不能把本地任务状态闭环写成真实 provider 或完整 `450+` replay runtime |
...
| Evidence ops | M4 acceptance harness 与 Dashboard report console | `scripts/m4_acceptance_gate.ps1` 已聚合 live truth、runtime adapter、provider、SessionBundle 和 profile-browser comparison；Dashboard 已展示 M4、release、runtime、profile/browser、remote proxy/TLS、headed、Camoufox、provider、SessionBundle、taxonomy 和 external distribution 报告；最新 M4 report 为 `expected_blocked`，`failed=0`；M4.2 workflow task center 状态闭环已有本地代码和测试证据 | M4 gate 保持 `failed=0`；同份 desktop/profile comparison、provider credential smoke、远程代理/TLS、跨机器 SessionBundle 按真实报告逐项从 `expected_blocked` 转为 passed |
