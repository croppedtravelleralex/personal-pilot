...
| Evidence ops | M4 acceptance harness 与 Dashboard report console | `scripts/m4_acceptance_gate.ps1` 已聚合 live truth、runtime adapter、provider、SessionBundle 和 profile-browser comparison；Dashboard 已展示 M4、release、runtime、profile/browser、remote proxy/TLS、headed、Camoufox、provider、SessionBundle、taxonomy 和 external distribution 报告；最新 M4 report 为 `expected_blocked`，`failed=0` | M4 gate 保持 `failed=0`；同份 desktop/profile comparison、provider credential smoke、远程代理/TLS、跨机器 SessionBundle 按真实报告逐项从 `expected_blocked` 转为 passed |
