...
## 2026-04-30 代理池清理视图与 IP 健康回退

- UX-008 / Done：代理池增加按状态快速查看与批量清理入口，覆盖可用、高延迟、测速失败/超时、IP 健康失败、IP 未检测、未测速；当前视图可直接全选后删除，内置代理仍受保护。
- BE-012 / Done：IP 健康检测从单一 IPPure 改为 `IPPure -> ip-api -> HTTPS canary` fallback；元数据接口失败但真实 HTTPS 可用时，不再把代理误判为完全不可用。
- MTN-007 / Done：免费代理默认源增加 6 个已验证 raw 渠道，但默认候选上限和并发不扩大；继续以真实检测结果控制入库质量。
