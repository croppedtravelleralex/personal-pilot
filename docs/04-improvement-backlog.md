...
| Fingerprint | 从 `12` runtime projected fields 扩展 runtime depth | 已扩到 `26` projected fields（`25` control-supported + derived `platform`）；P7 已新增 WebRTC/canvas/audio/leak observed audit；P14 已新增 `450` fingerprint signal taxonomy seed；2026-05-28 已新增环境注入编译器和单次 CDP 新文档注入入口，覆盖 browser API/canvas/timezone/WebGL/media devices 基础 hook；`roadmap_evidence_smoke` 已证明 1.2/1.3/1.4 本机实现覆盖，`profile_browser_environment_probe.mjs` 已用真实 Chromium profile-browser 观测 1.2/1.3/1.4 passed；本轮已把环境注入接入实例 debug-ready 流程，并把 audit 扩到 8 个注入族；`taxonomy_coverage_materialize.ps1` 已物化 `450 / 450` 条 fingerprint signal contract；profile-browser comparison 最新为 `blocked_missing_desktop_webview_report`，缺同份 desktop-webview 对照报告 | declared / applied / observed 可解释且可验证；taxonomy seed/materialized contract 不得计入 full observed coverage；下一步补同份 desktop/profile comparison 与更完整 detector matrix |
...
| Evidence ops | M4 acceptance harness 与 Dashboard report console | `scripts/m4_acceptance_gate.ps1` 已聚合 live truth、runtime adapter、provider、SessionBundle 和 profile-browser comparison；Dashboard 已展示 M4、release、runtime、profile/browser、remote proxy/TLS、headed、Camoufox、provider、SessionBundle、taxonomy 和 external distribution 报告；最新 M4 report 为 `expected_blocked`，`passed=3`、`expectedBlocked=4`、`failed=0`；M4.2 workflow task center、M4.3 typed primitives、M4.4 provider dry-run contract 均已有本地代码/报告证据 | M4 gate 保持 `failed=0`；同份 desktop/profile comparison、provider credential smoke、远程代理/TLS、跨机器 SessionBundle 按真实报告逐项从 `expected_blocked` 转为 passed |
