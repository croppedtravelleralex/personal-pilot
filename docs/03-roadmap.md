...
6. Retire duplicate UI paths
   ...
   - 下一步不再做旧 UI 复活式迁移；改为在截图主线 UI 内继续缩小动态 facade 面积，保持 `tauriWailsBridge` 作为过渡兼容层，优先把剩余 workbench/core bridge API 迁到统一 typed facade。
