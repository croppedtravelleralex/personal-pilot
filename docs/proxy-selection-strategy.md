...
- trust score 已经吸收了原始分数，尾部不再单独看 `score`
- 最近没被用过的代理，会更容易被轮到
...
- `PP_PROXY_STALE_AFTER_SECONDS`
- `PP_PROXY_RECENT_FAILURE_HEAVY_WINDOW_SECONDS`
- `PP_PROXY_RECENT_FAILURE_LIGHT_WINDOW_SECONDS`
- `PP_PROXY_PROVIDER_FAILURE_MARGIN`
- `PP_PROXY_PROVIDER_REGION_CLUSTER_WINDOW_SECONDS`
- `PP_PROXY_PROVIDER_REGION_CLUSTER_COUNT`
- `PP_PROXY_RAW_SCORE_WEIGHT_TENTHS`
- `PP_PROXY_VERIFY_OK_BONUS`
- `PP_PROXY_VERIFY_GEO_MATCH_BONUS`
- `PP_PROXY_SMOKE_UPSTREAM_OK_BONUS`
- `PP_PROXY_VERIFY_FAILED_HEAVY_PENALTY`
- `PP_PROXY_VERIFY_FAILED_LIGHT_PENALTY`
- `PP_PROXY_VERIFY_FAILED_BASE_PENALTY`
- `PP_PROXY_MISSING_VERIFY_PENALTY`
- `PP_PROXY_STALE_VERIFY_PENALTY`
...
当前它已经接入 selection 主排序，实际排序口径已经进入 **trust score 优先、尾部资源均衡** 的阶段。也就是说，selection 已不再是“先按 rule 分层、最后才看 score”，而是开始由统一 trust score 承担主排序责任；后续仍应继续把剩余分散语义进一步并入 trust/risk score，减少对尾部排序项的依赖。
