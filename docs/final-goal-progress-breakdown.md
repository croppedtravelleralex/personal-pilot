...
- `SessionBundle` profile-scoped export、import preflight、non-destructive restore contract 已落地；真实 restore write path 和 profile portability smoke 尚未落地
- Validation Board 已有 native DNS/transport reports、desktop WebView scoped probes、profile runtime probe contract、report history 和 profile-level export；真实 Lightpanda/CDP operator smoke 尚未完成
- 当前行为运行时只支持 `13` 个真实原语
