...
- 涉及 M4 UI error boundary 时，M4 gate v23 的 `ui_error_boundary_contract` 只代表 Dashboard、Dashboard API、Settings、Automation、NaturalLanguageTask、TagManagement、RecordingDetailModal、LaunchApiDocsPage、BrowserList/Detail/Edit、BrowserSettingsModal、QuickLaunchModal，以及 CoreManagementPage 这些选定 surfaces 的错误处理已改用 `unknown` guard / typed cast guard；不得写成全仓 `any` catch 已清零、所有 UI 错误边界已统一或 `tauriWailsBridge` 已移除。
