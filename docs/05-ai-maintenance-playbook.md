...
- 涉及 M4 total gate 时，`passed_with_expected_external_blockers` 代表本地 M4 合同可用且没有 failed gate；仍不得把 provider、远程代理/TLS、AdsPower refresh 或完整 observed/replay coverage 写成已完成。跨机器 SessionBundle、外部分发 smoke 和 release performance budget 已取消，不再当作 expected blocker。
