...
async fn auto_selection_result_exposes_trust_score_components_and_candidate_preview() {
    ...
    assert!(selection_artifact.get("summary").and_then(|v| v.as_str()).map(|s| s.contains("selected this proxy by a") && s.contains("biggest reasons")).unwrap_or(false));
...
async fn status_latest_execution_summaries_include_selection_decision_artifact() {
    ...
    assert!(summary.contains("selected this proxy by a"));
    assert!(summary.contains("biggest reasons"));
